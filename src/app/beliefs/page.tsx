...
export default async function BeliefsPage() {
  ...
  // Pre-resolve each conviction (CMS overrides applied) once, so the desktop list
  // and the mobile <BeliefsAccordion> render identical copy from a single source.
  const items = beliefs.map((b, i) => ({
    id: b.id,
    n: i + 1,
    title: tx(t, `beliefs-item-${b.id}-title`, b.title),
    paragraphs: b.paragraphs.map((para, pi) =>
      tx(t, `beliefs-item-${b.id}-p${pi}`, para)
    ),
  }));
