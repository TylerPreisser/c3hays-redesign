...
# the production SSR shape), sourced by scripts/ci-local.sh inside a CLEAN worktree ($WT). The first failing step
# ends the gate, as the workflow stops. Keep it in step with ci.yml, which stays in the repo on workflow_dispatch
